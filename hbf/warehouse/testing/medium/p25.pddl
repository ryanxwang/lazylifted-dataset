;; boxes=140, remove_proportion=0.05, bases=56, instance_id=25, output=./testing/medium, seed=2048

(define (problem warehouse-25)
    (:domain warehouse)
    (:objects
        box1 box2 box3 box4 box5 box6 box7 box8 box9 box10 box11 box12 box13 box14 box15 box16 box17 box18 box19 box20 box21 box22 box23 box24 box25 box26 box27 box28 box29 box30 box31 box32 box33 box34 box35 box36 box37 box38 box39 box40 box41 box42 box43 box44 box45 box46 box47 box48 box49 box50 box51 box52 box53 box54 box55 box56 box57 box58 box59 box60 box61 box62 box63 box64 box65 box66 box67 box68 box69 box70 box71 box72 box73 box74 box75 box76 box77 box78 box79 box80 box81 box82 box83 box84 box85 box86 box87 box88 box89 box90 box91 box92 box93 box94 box95 box96 box97 box98 box99 box100 box101 box102 box103 box104 box105 box106 box107 box108 box109 box110 box111 box112 box113 box114 box115 box116 box117 box118 box119 box120 box121 box122 box123 box124 box125 box126 box127 box128 box129 box130 box131 box132 box133 box134 box135 box136 box137 box138 box139 box140 - box
        base1 base2 base3 base4 base5 base6 base7 base8 base9 base10 base11 base12 base13 base14 base15 base16 base17 base18 base19 base20 base21 base22 base23 base24 base25 base26 base27 base28 base29 base30 base31 base32 base33 base34 base35 base36 base37 base38 base39 base40 base41 base42 base43 base44 base45 base46 base47 base48 base49 base50 base51 base52 base53 base54 base55 base56 - base
    )
    (:init
        (to-remove box63)
        (to-remove box74)
        (to-remove box85)
        (to-remove box92)
        (to-remove box98)
        (to-remove box128)
        (to-remove box133)
        (clear box69)
        (on box69 box32)
        (on box32 box127)
        (on box127 box2)
        (on box2 box68)
        (on box68 box116)
        (on-base box116 base1)
        (clear box60)
        (on box60 box49)
        (on-base box49 base2)
        (clear box7)
        (on-base box7 base3)
        (clear box72)
        (on box72 box66)
        (on box66 box74)
        (on box74 box122)
        (on-base box122 base4)
        (clear box88)
        (on box88 box48)
        (on-base box48 base5)
        (clear box9)
        (on box9 box95)
        (on box95 box120)
        (on box120 box54)
        (on box54 box47)
        (on-base box47 base6)
        (clear box65)
        (on box65 box117)
        (on box117 box46)
        (on-base box46 base7)
        (clear box135)
        (on box135 box27)
        (on box27 box4)
        (on box4 box28)
        (on-base box28 base8)
        (clear box52)
        (on box52 box50)
        (on box50 box8)
        (on-base box8 base9)
        (clear box80)
        (on box80 box29)
        (on-base box29 base10)
        (clear box123)
        (on box123 box140)
        (on box140 box44)
        (on box44 box105)
        (on-base box105 base11)
        (clear box131)
        (on-base box131 base12)
        (clear box101)
        (on-base box101 base13)
        (clear box42)
        (on box42 box107)
        (on box107 box17)
        (on box17 box108)
        (on box108 box12)
        (on-base box12 base14)
        (clear box109)
        (on-base box109 base15)
        (clear box86)
        (on box86 box41)
        (on box41 box59)
        (on box59 box15)
        (on box15 box31)
        (on-base box31 base16)
        (clear box62)
        (on-base box62 base17)
        (clear box125)
        (on box125 box82)
        (on-base box82 base18)
        (clear box22)
        (on box22 box100)
        (on box100 box98)
        (on-base box98 base19)
        (clear-base base20)
        (clear box126)
        (on box126 box96)
        (on box96 box130)
        (on box130 box38)
        (on box38 box129)
        (on box129 box40)
        (on-base box40 base21)
        (clear box128)
        (on-base box128 base22)
        (clear box124)
        (on-base box124 base23)
        (clear box84)
        (on box84 box137)
        (on-base box137 base24)
        (clear box39)
        (on box39 box132)
        (on box132 box45)
        (on box45 box90)
        (on box90 box118)
        (on-base box118 base25)
        (clear-base base26)
        (clear box138)
        (on-base box138 base27)
        (clear box81)
        (on box81 box19)
        (on-base box19 base28)
        (clear box25)
        (on box25 box23)
        (on-base box23 base29)
        (clear box30)
        (on box30 box106)
        (on-base box106 base30)
        (clear box64)
        (on box64 box111)
        (on box111 box112)
        (on box112 box67)
        (on-base box67 base31)
        (clear box71)
        (on-base box71 base32)
        (clear box89)
        (on box89 box33)
        (on box33 box136)
        (on box136 box94)
        (on box94 box51)
        (on-base box51 base33)
        (clear-base base34)
        (clear box70)
        (on box70 box113)
        (on box113 box63)
        (on box63 box57)
        (on box57 box93)
        (on-base box93 base35)
        (clear box61)
        (on-base box61 base36)
        (clear box119)
        (on box119 box34)
        (on-base box34 base37)
        (clear box36)
        (on box36 box121)
        (on-base box121 base38)
        (clear box18)
        (on-base box18 base39)
        (clear box26)
        (on box26 box43)
        (on box43 box5)
        (on box5 box37)
        (on-base box37 base40)
        (clear box55)
        (on box55 box91)
        (on-base box91 base41)
        (clear box114)
        (on box114 box73)
        (on box73 box87)
        (on box87 box10)
        (on-base box10 base42)
        (clear box58)
        (on-base box58 base43)
        (clear box83)
        (on box83 box20)
        (on box20 box139)
        (on box139 box56)
        (on-base box56 base44)
        (clear box115)
        (on box115 box53)
        (on box53 box11)
        (on-base box11 base45)
        (clear-base base46)
        (clear box21)
        (on box21 box75)
        (on box75 box35)
        (on-base box35 base47)
        (clear box133)
        (on box133 box76)
        (on-base box76 base48)
        (clear box102)
        (on box102 box85)
        (on box85 box104)
        (on-base box104 base49)
        (clear-base base50)
        (clear box77)
        (on-base box77 base51)
        (clear box99)
        (on box99 box3)
        (on-base box3 base52)
        (clear box1)
        (on box1 box13)
        (on box13 box134)
        (on box134 box79)
        (on box79 box92)
        (on-base box92 base53)
        (clear box16)
        (on box16 box24)
        (on box24 box110)
        (on-base box110 base54)
        (clear box97)
        (on box97 box14)
        (on-base box14 base55)
        (clear box78)
        (on box78 box103)
        (on box103 box6)
        (on-base box6 base56)
    )
    (:goal (and 
        (removed box63)
        (removed box74)
        (removed box85)
        (removed box92)
        (removed box98)
        (removed box128)
        (removed box133)
    ))
)