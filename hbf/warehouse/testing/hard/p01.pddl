;; boxes=150, remove_proportion=0.05, bases=60, instance_id=1, output=./testing/hard, seed=2024

(define (problem warehouse-01)
    (:domain warehouse)
    (:objects
        box1 box2 box3 box4 box5 box6 box7 box8 box9 box10 box11 box12 box13 box14 box15 box16 box17 box18 box19 box20 box21 box22 box23 box24 box25 box26 box27 box28 box29 box30 box31 box32 box33 box34 box35 box36 box37 box38 box39 box40 box41 box42 box43 box44 box45 box46 box47 box48 box49 box50 box51 box52 box53 box54 box55 box56 box57 box58 box59 box60 box61 box62 box63 box64 box65 box66 box67 box68 box69 box70 box71 box72 box73 box74 box75 box76 box77 box78 box79 box80 box81 box82 box83 box84 box85 box86 box87 box88 box89 box90 box91 box92 box93 box94 box95 box96 box97 box98 box99 box100 box101 box102 box103 box104 box105 box106 box107 box108 box109 box110 box111 box112 box113 box114 box115 box116 box117 box118 box119 box120 box121 box122 box123 box124 box125 box126 box127 box128 box129 box130 box131 box132 box133 box134 box135 box136 box137 box138 box139 box140 box141 box142 box143 box144 box145 box146 box147 box148 box149 box150 - box
        base1 base2 base3 base4 base5 base6 base7 base8 base9 base10 base11 base12 base13 base14 base15 base16 base17 base18 base19 base20 base21 base22 base23 base24 base25 base26 base27 base28 base29 base30 base31 base32 base33 base34 base35 base36 base37 base38 base39 base40 base41 base42 base43 base44 base45 base46 base47 base48 base49 base50 base51 base52 base53 base54 base55 base56 base57 base58 base59 base60 - base
    )
    (:init
        (to-remove box47)
        (to-remove box52)
        (to-remove box68)
        (to-remove box78)
        (to-remove box105)
        (to-remove box121)
        (to-remove box149)
        (clear box123)
        (on-base box123 base1)
        (clear box69)
        (on-base box69 base2)
        (clear box65)
        (on-base box65 base3)
        (clear box37)
        (on-base box37 base4)
        (clear box148)
        (on box148 box18)
        (on box18 box119)
        (on-base box119 base5)
        (clear box131)
        (on-base box131 base6)
        (clear box120)
        (on-base box120 base7)
        (clear box43)
        (on box43 box81)
        (on-base box81 base8)
        (clear box91)
        (on box91 box46)
        (on box46 box113)
        (on-base box113 base9)
        (clear box125)
        (on box125 box32)
        (on box32 box98)
        (on-base box98 base10)
        (clear box108)
        (on-base box108 base11)
        (clear box122)
        (on-base box122 base12)
        (clear box132)
        (on box132 box53)
        (on box53 box146)
        (on box146 box92)
        (on-base box92 base13)
        (clear box59)
        (on box59 box12)
        (on box12 box116)
        (on box116 box34)
        (on box34 box22)
        (on box22 box64)
        (on-base box64 base14)
        (clear box67)
        (on box67 box63)
        (on box63 box87)
        (on box87 box130)
        (on-base box130 base15)
        (clear box88)
        (on box88 box128)
        (on box128 box135)
        (on box135 box2)
        (on-base box2 base16)
        (clear box99)
        (on box99 box71)
        (on box71 box139)
        (on-base box139 base17)
        (clear-base base18)
        (clear box134)
        (on-base box134 base19)
        (clear box149)
        (on box149 box150)
        (on box150 box13)
        (on-base box13 base20)
        (clear box57)
        (on box57 box74)
        (on box74 box48)
        (on box48 box54)
        (on-base box54 base21)
        (clear box102)
        (on box102 box82)
        (on box82 box16)
        (on box16 box114)
        (on box114 box50)
        (on box50 box103)
        (on-base box103 base22)
        (clear box39)
        (on box39 box129)
        (on box129 box51)
        (on box51 box7)
        (on-base box7 base23)
        (clear box93)
        (on box93 box90)
        (on-base box90 base24)
        (clear box100)
        (on box100 box49)
        (on-base box49 base25)
        (clear-base base26)
        (clear box145)
        (on box145 box36)
        (on box36 box56)
        (on box56 box8)
        (on box8 box41)
        (on box41 box79)
        (on box79 box62)
        (on-base box62 base27)
        (clear box143)
        (on box143 box55)
        (on-base box55 base28)
        (clear box126)
        (on-base box126 base29)
        (clear box137)
        (on box137 box89)
        (on box89 box106)
        (on box106 box26)
        (on box26 box42)
        (on-base box42 base30)
        (clear box140)
        (on-base box140 base31)
        (clear box94)
        (on box94 box6)
        (on-base box6 base32)
        (clear box73)
        (on-base box73 base33)
        (clear box17)
        (on box17 box9)
        (on box9 box141)
        (on-base box141 base34)
        (clear box33)
        (on box33 box1)
        (on box1 box14)
        (on-base box14 base35)
        (clear-base base36)
        (clear box58)
        (on box58 box76)
        (on-base box76 base37)
        (clear box117)
        (on-base box117 base38)
        (clear box85)
        (on box85 box138)
        (on box138 box95)
        (on-base box95 base39)
        (clear box11)
        (on box11 box80)
        (on box80 box101)
        (on-base box101 base40)
        (clear box3)
        (on box3 box40)
        (on box40 box97)
        (on-base box97 base41)
        (clear box84)
        (on box84 box31)
        (on-base box31 base42)
        (clear box144)
        (on-base box144 base43)
        (clear box109)
        (on box109 box118)
        (on-base box118 base44)
        (clear box23)
        (on box23 box110)
        (on box110 box136)
        (on-base box136 base45)
        (clear box127)
        (on box127 box78)
        (on box78 box15)
        (on box15 box28)
        (on box28 box105)
        (on box105 box104)
        (on-base box104 base46)
        (clear box44)
        (on box44 box10)
        (on box10 box19)
        (on box19 box25)
        (on-base box25 base47)
        (clear box66)
        (on box66 box45)
        (on box45 box5)
        (on-base box5 base48)
        (clear box68)
        (on box68 box24)
        (on box24 box47)
        (on-base box47 base49)
        (clear box38)
        (on box38 box111)
        (on box111 box86)
        (on box86 box75)
        (on box75 box20)
        (on-base box20 base50)
        (clear box52)
        (on-base box52 base51)
        (clear box133)
        (on box133 box77)
        (on-base box77 base52)
        (clear box4)
        (on-base box4 base53)
        (clear box96)
        (on box96 box29)
        (on box29 box121)
        (on box121 box147)
        (on-base box147 base54)
        (clear box83)
        (on box83 box72)
        (on-base box72 base55)
        (clear box30)
        (on box30 box21)
        (on box21 box35)
        (on box35 box107)
        (on box107 box112)
        (on box112 box61)
        (on box61 box70)
        (on-base box70 base56)
        (clear box142)
        (on-base box142 base57)
        (clear box60)
        (on box60 box124)
        (on-base box124 base58)
        (clear-base base59)
        (clear box27)
        (on box27 box115)
        (on-base box115 base60)
    )
    (:goal (and 
        (removed box47)
        (removed box52)
        (removed box68)
        (removed box78)
        (removed box105)
        (removed box121)
        (removed box149)
    ))
)