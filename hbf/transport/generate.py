...
def dense_density(locations: int) -> float:
    ...
    num_edges = random.randint(
        int(0.5 * max_possible_edges), int(0.8 * max_possible_edges)
    )
...
CONFIG = {
    "sparse/training": {
        "vehicles": [int(3 + 3 / 90 * i) for i in range(90)],  # 3 to 6
        "packages": [int(1 + 14 / 90 * i) for i in range(90)],  # 1 to 15
        "locations": [int(7 + 8 / 90 * i) for i in range(90)],  # 7 to 15
        "max_capacity": [2 for _ in range(90)],  # 2
...
def main():
    parser = ArgumentParser(
        description="Transport instances generator for various densities"
    )
    parser.add_argument(
        "-o",
        "--output",
